...
def summarize(name: str, gaps: np.ndarray, n_nan: int) -> dict:
    ...
    print(
        f"  [{name:<28}] mean={mean:.6g}  std={std:.6g}  "
...
def main() -> int:
    ...
    with tee_stdout(run_dir / "log.txt"):
        ...
        print(
            f"  regimes : pre_crash={int((regimes == 0).sum())} crash={int((regimes == 1).sum())}"
        )
        ...
        within_reg_pre = WithinRegimeSampler(
            regimes, target_regime=0, max_n_per_half=WITHIN_REGIME_N
        )
        within_reg_cra = WithinRegimeSampler(
            regimes, target_regime=1, max_n_per_half=WITHIN_REGIME_N
        )
        arm_specs = [
        ...
        for bkey, gap_fn in BUCKETS.items():
            ...
            for sampler_name, regime, sampler, n in arm_specs:
                ...
                t0 = time.time()
                gaps, n_nan = run_arm(bkey, gap_fn, real, sampler, n, N_DRAWS, seed)
