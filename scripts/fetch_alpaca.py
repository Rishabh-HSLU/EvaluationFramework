...
def download_corpus(
    ...
    try:
        ...
        raise ImportError("alpaca-py is required: pip install 'alpaca-py>=0.13'") from exc
    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            ...
            log_df = pd.concat(
                [
                    log_df,
                    pd.DataFrame([{"ticker": ticker, "bars": n_bars, "status": "ok", "note": ""}]),
                ],
                ignore_index=True,
            )
            ...
            log_df = pd.concat(
                [
                    log_df,
                    pd.DataFrame(
                        [{"ticker": ticker, "bars": 0, "status": "error", "note": str(exc)}]
                    ),
                ],
                ignore_index=True,
            )
