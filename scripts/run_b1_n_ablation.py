...
def main() -> int:
    ...
    with tee_stdout(run_dir / "log.txt"):
        ...
        seeds = [int(s.generate_state(1, dtype=np.uint32)[0]) for s in seq.spawn(len(N_VALUES))]

        rows = []
        for n, seed in zip(N_VALUES, seeds, strict=True):
            t0 = time.time()
            ...
            print(
                f"  N={n:>3}  mean={mean:.6f}  std={std:.6f}  "
