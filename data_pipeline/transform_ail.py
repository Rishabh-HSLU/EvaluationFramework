...
def process_ticker(tau_ret: np.ndarray, s: np.ndarray, window_len: int) -> list[np.ndarray]:
    ...
    return [
        deseas[i * window_len : (i + 1) * window_len].astype(np.float32) for i in range(n_windows)
    ]
...
def main() -> None:
    ...
    p.add_argument("--parquet", type=Path, required=True, help="Path to the AIL synthetic parquet.")
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/output_data"),
